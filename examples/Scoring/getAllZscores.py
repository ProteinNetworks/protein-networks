...
try:
    for i, pdb in enumerate(pdbs):
        try:
            ...
            if maxI != -1:
                # Get the z-score.
                z = proteinnetworks.insight.getZScore(
                    pfamDomains, np.asarray(
                        partition.data[maxI], dtype=int))
                zscores.append([pdb, z])
        except FileNotFoundError as e:
            print(str(e))
        except ValueError as e:
            print(str(e))
except KeyboardInterrupt:
    with open("allZScores.dat", mode='w') as flines:
        flines.write("\n".join(" ".join(map(str, x)) for x in zscores))

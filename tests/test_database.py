...
def test_database_getnumberofdocuments(mock_database):
    ...
    """
    db = proteinnetworks.database.Database(password="bla")
    assert db.getNumberOfDocuments() == 17
...
"""
-------------------------------------------------------------------------
ALL THE TESTS AGAIN, BUT THIS TIME WITH THE LOCAL DATABASE
-------------------------------------------------------------------------
"""


def test_local_database_initialise():
    """
    Meta-test, to check that pytest and Travis are behaving.
...
def test_local_database_getnumberofdocuments():
    """
    Test the getNumberOfDocuments function.
    ...
    # Check that the only document is the one we put there
    assert db.getNumberOfDocuments() == 1
    ...
    db.depositPartition(**depositionArgs)
    assert db.getNumberOfDocuments() == 2
...
def test_local_database_depositpartition_success(mock_database):
    """Test the depositPartition function when all arguments are correct."""
    db = proteinnetworks.database.Database(local=True)
    depositionArgs = {
        'pdbref': '2vc5',
...
def test_local_database_depositpartition_doc_already_present(mock_database):
    """Test what happens if you try to add a document that's already there."""
    db = proteinnetworks.database.Database(local=True)
    depositionArgs = {
        'pdbref': '2vc5',
...
def test_local_database_extractdocumentgivenid_normal(mock_database):
    """Assert that the database returns the right document given an id."""
    db = proteinnetworks.database.Database(local=True)
    depositionArgs = {
        'pdbref': '2vc5',
    ...
    edgelistId = db.depositEdgelist(**depositionArgs)
    doc = db.extractDocumentGivenId(edgelistId)
    assert doc


# def test_local_database_extractdocumentgivenid_missing(mock_database):
#     """Assert that the database returns None if the document to be found is missing."""
#     db = proteinnetworks.database.Database(local=True)
#     doc = db.extractDocumentGivenId("58dbe03fef677d54224a01d7")
#     assert not doc
...
# def test_local_database_extractdocumentgivenid_malformed(mock_database):
#     """Assert that the database returns an IOError if the id is malformed."""
#     db = proteinnetworks.database.Database(local=True)
#     with pytest.raises(IOError):
#         db.extractDocumentGivenId("bla")
...
# def test_local_database_extractpartition_success(mock_database):
#     """Assert that a partition we know to exist can be successfully pulled."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '1ubq',
...
# def test_local_database_extractpartition_missing(mock_database):
#     """Test that a partition we know not to exist (but with correct formatting) returns None."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '1ubq',
...
# def test_local_database_extractpartition_malformed_pdb(mock_database):
#     """Assert that depositPartition handles bad PDB references correctly."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': 'blablabla',
...
# def test_local_database_extractpartition_edgelistid_invalid(mock_database):
#     """Assert that an malformed ObjectId is rejected correctly."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '1ubq',
...
# def test_local_database_extractpartition_edgelistid_incorrect(mock_database):
#     """
#     Test valid, but incorrect ObjectIDs.
...
# def test_local_database_extractpartition_detectionmethod_invalid(mock_database):
#     """Assert that if "detectionmethod" isn't "AFG" or "Infomap" then an exception is raised."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '1ubq',
...
# def test_local_database_extractpartition_r_invalid(mock_database):
#     """Test that invalid r values (aka not a float) are treated properly."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '1ubq',
...
# def test_local_database_extractpartition_N_invalid(mock_database):
#     """Test that invalid N values (aka not an integer) are treated properly."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '1ubq',
...
# def test_local_database_extractpartition_r_and_N_not_both_missing(mock_database):
#     """Test that if r and N are simultaneously -1 an error is raised."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '1ubq',
...
# def test_local_database_extractpartition_r_and_N_not_both_present(mock_database):
#     """Test that if r and N are simultaneously not -1 an error is raised."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '1ubq',
...
# def test_local_database_extractpdbfile_success(mock_database):
#     """Assert that a correct PDB reference returns the correct PDB file, correctly formatted."""
#     db = proteinnetworks.database.Database(local=True)
#     pdbref = "1ubq"
#     pdbfile = db.extractPDBFile(pdbref)
...
# def test_local_database_extractpdbfile_missing(mock_database):
#     """Assert that if the PDB reference is valid but not found, nothing is returned."""
#     db = proteinnetworks.database.Database(local=True)
#     pdbref = "1abc"
#     pdbfile = db.extractPDBFile(pdbref)
...
# def test_local_database_extractpdbfile_error(mock_database):
#     """Assert that if the PDB reference is invalid, an IOError is thrown."""
#     db = proteinnetworks.database.Database(local=True)
#     pdbref = 5
#     with pytest.raises(IOError):
...
# def test_local_database_fetchpdbfilefromweb_success(mock_database, mock_urlopen):
#     """Assert that a valid PDB reference can be fetched, stripped, deposited and returned."""
#     db = proteinnetworks.database.Database(local=True)
#     pdbref = "3rty"
#     pdbfile = db.fetchPDBFileFromWeb(pdbref)
...
# def test_local_database_fetchpdbfilefromweb_error(mock_database, mock_urlopen):
#     """Assert that a malformed PDB reference will cause an IOError."""
#     db = proteinnetworks.database.Database(local=True)
#     pdbref = 5
#     with pytest.raises(IOError):
...
# def test_local_database_fetchpdbfilefromweb_pdbfilealreadyindatabase(mock_database, mock_urlopen):
#     """
#     Try to fetch a PDB file that's already in the database.
...
# def test_local_database_depositedgelist_normal(mock_database):
#     """Test that if all arguments are correct, the edgelist is deposited succesfully."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vc5',
...
# def test_local_database_depositedgelist_already_present(mock_database):
#     """Test that if the edgelist is already present, an exception is thrown."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vcr',
...
# def test_local_database_depositedgelist_pdbref_invalid(mock_database):
#     """Test that a malformed PDB reference is rejected."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': 'blablabla',
...
# def test_local_database_depositedgelist_edgelisttype_invalid(mock_database):
#     """Test that if the edgelisttype isn't "atomic" or "residue" it is rejected."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vc5',
...
# def test_local_database_depositedgelist_hydrogenstatus_invalid(mock_database):
#     """Test that if the hydrogenstatus isn't "noH", "Hatoms" or "Hbonds" it is rejected."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vc5',
...
# def test_local_database_depositedgelist_scaling_invalid_negative(mock_database):
#     """Assert that scaling values must be 0.0."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vc5',
...
# def test_local_database_depositedgelist_scaling_invalid_notnumber(mock_database):
#     """Assert that scaling values must be floats."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vc5',
...
# def test_local_database_depositedgelist_edges_not_array(mock_database):
#     """Assert that the edgelist is rejected if it is not a _ x 3 array of numbers."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vc5',
...
# def test_local_database_depositedgelist_edges_not_correctly_indexed(mock_database):
#     """Assert that the edge labelling starts from 1."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vc5',
...
# def test_local_database_depositedgelist_edges_no_self_loops(mock_database):
#     """Assert no self-loops."""
#     db = proteinnetworks.database.Database(local=True)
#     depositionArgs = {
#         'pdbref': '2vc5',
...
# def test_local_database_extractedgelist_success(mock_database):
#     """Ensure successful extraction if all arguments correct."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '2vcr',
...
# def test_local_database_extractedgelist_edgelist_missing(mock_database):
#     """Ensure None returned if edgelist missing."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': '2vcr',
...
# def test_local_database_extractedgelist_pdbref_invalid(mock_database):
#     """Ensure exception thrown if pdbref malformed."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': 5,
...
# def test_local_database_extractedgelist_edgelisttype_invalid(mock_database):
#     """Ensure exception thrown if edgelisttype malformed."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': 5,
...
# def test_local_database_extractedgelist_hydrogenstatus_invalid(mock_database):
#     """Ensure exception thrown if hydrogenstatus not from accepted options."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': 5,
...
# def test_local_database_extractedgelist_scaling_invalid(mock_database):
#     """Ensure exception thrown if scaling invalid."""
#     db = proteinnetworks.database.Database(local=True)
#     extractionArgs = {
#         'pdbref': 5,
